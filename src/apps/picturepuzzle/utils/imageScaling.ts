 ...
 * @param tileValue The value of the tile (1-15)
 * @param imageWidth Original image width
 * @param imageHeight Original image height
...
export const getTileBackgroundStyle = (
  ...
  // Calculate grid position based on tile value
  // For tiles 1-15, we want value 1 to show top-left of image, value 15 to show bottom-right excluding last position
  const row = Math.floor((tileValue - 1) / 4);
  const col = (tileValue - 1) % 4;
