...
- Blog: rebuilt blog is live in prod on `main`. Content lives in `blog/content/{section}/{post}/index.md`; homepage is hand-authored at `blog/content/home.md`. Posts support a `draft: true` frontmatter flag (skipped by `build.js` when generating the manifest); the `demo` section's three placeholder posts are all marked draft so they stay in the repo but don't ship. Local preview requires `npx wrangler pages dev .` (not a plain static server) since clean URLs depend on `_redirects`. See session-005 for full detail.
