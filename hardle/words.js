...
function createCyclicGenerator(wordList, offset = 0) {
...
// Note: These internal functions have been moved to the bottom of the file
// and wrapped with proper exports

// Convert valid guesses to a Set for O(1) lookup
const VALID_WORDS_SET = new Set(VALID_GUESSES);
...
function isValidWord(word) {
  return VALID_WORDS_SET.has(word.toLowerCase());
}

/**
 * A hash function for strings to create a numeric seed
 ...
 * @returns {string} - The daily word (uppercase)
 */
function getDailyWord(date) {
  return getDailyWordInternal(ANSWER_WORDS, date);
}

/**
 * Get a random word for Randle mode
 * @param {string|number} seed - Optional seed for randomization
 * @returns {string} - A random word (uppercase)
 */
function getRandomWord(seed) {
  return getRandomWordInternal(ANSWER_WORDS, seed);
}

// Rename the internal functions
function getDailyWordInternal(wordList, date = new Date()) {
  ...
  // Final position is offset + cycle position, modulo length
  const wordIndex = (cycleOffset + cyclePosition) % wordList.length;

  return wordList[wordIndex].toUpperCase();
}

function getRandomWordInternal(wordList, seed) {
  ...
  return wordList[startIndex].toUpperCase();
}
