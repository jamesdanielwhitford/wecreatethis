...
// feel), while a background fetch revalidates the cache. marked.js CDN
// (pre-cached at install) is cache-first so the markdown renderer keeps
// working offline.
  ...
  if (url.hostname === 'cdn.jsdelivr.net') {
    event.respondWith(
      caches.open(CACHE_NAME).then(async (cache) => {
        ...
        if (response.ok) await cache.put(normalizeUrl(event.request.url), await cleanResponse(response.clone()));
        return response;
      })
    );
    return;
