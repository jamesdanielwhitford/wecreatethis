...
- **Root (`/`) - the blog.** Moved here from `blog/` (merged to `main`, live in prod as of session 021, 2026-08-08) so the site is primarily a blog; the old app-hub grid (index.html/sw.js/manifest.json/styles.css/icons) is archived, not deleted, at `archive/app-hub/`. Markdown posts with offline support, organized in folders on disk (`content/{group...}/{post}/index.md`) but served flat at `/{post-slug}` - folder structure never appears in the URL (session 028, not yet pushed - see [sessions/overview.md](sessions/overview.md)). Homepage is a generated tag-filter hub (intro + featured project tiles from `homepage.json` + a tag chip row + the full post list, `?tag=x` filtering), not the old hand-authored index page. Bio modal has a real photo, a live Pretoria/GMT+2 clock, and a CV link; site has its first favicon (`favicon.svg`). See [CLAUDE-blog.md](CLAUDE-blog.md) for details, including how its service worker deliberately differs from the repo-wide SW rules below (root `sw.js` IS the blog's SW now, there is no separate blog-scoped one).
