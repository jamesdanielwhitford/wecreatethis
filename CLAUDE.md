...
- **Root (`/`) - the blog.** Moved here from `blog/` (merged to `main`, live in prod as of session 021, 2026-08-08) so the site is primarily a blog; the old app-hub grid (index.html/sw.js/manifest.json/styles.css/icons) is archived, not deleted, at `archive/app-hub/`. Markdown posts with offline support, organized in folders on disk (`content/{group...}/{post}/index.md`) but served flat at `/{post-slug}` - folder structure never appears in the URL (session 028). Homepage is a generated hub: intro (`content/home.md`) + real projects grouped into category sections, each rendered from `homepage.json` and clicking one collapses the page to just that project's posts (`?tag=x`) - no independent tag chip filter any more, project selection is the only filtering mechanism (session 029 replaced the original tag-chip design with this). As of session 030 the homepage only ever shows projects/posts that are actually finished - in-progress project stubs and unfinished posts are kept in the repo as `draft: true` rather than shown as placeholders. Bio modal has a real photo, a live Pretoria/GMT+2 clock, and a CV link; site has its first favicon (`favicon.svg`). Sessions 028-030 are all live in prod - see [sessions/overview.md](sessions/overview.md). See [CLAUDE-blog.md](CLAUDE-blog.md) for details, including how its service worker deliberately differs from the repo-wide SW rules below (root `sw.js` IS the blog's SW now, there is no separate blog-scoped one).
