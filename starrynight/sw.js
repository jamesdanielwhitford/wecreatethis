...
// feel), while a background fetch revalidates the cache. CDN assets
// (pre-cached at install) are cache-first. API calls (other cross-origin)
    ...
    caches.open(CACHE_NAME).then(async (cache) => {
      ...
      await Promise.all(CDN_ASSETS.map(async (url) => {
        try {
          const response = await fetch(url);
          if (response.ok) {
            await cache.put(url, await cleanResponse(response.clone()));
          }
        } catch (err) {
          console.warn('Failed to cache:', url, err);
        }
  ...
  if (url.hostname.includes('jsdelivr.net')) {
    event.respondWith((async () => {
      ...
      try {
        ...
        if (response.ok) await cache.put(url.href, await cleanResponse(response.clone()));
        return response;
      } catch (err) {
